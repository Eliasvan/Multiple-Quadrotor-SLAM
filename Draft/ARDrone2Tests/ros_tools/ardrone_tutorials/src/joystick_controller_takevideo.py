...
# and the TakeVideo class, which handles recording an image-sequence.
from drone_controller import BasicDroneController
from drone_video_display import DroneVideoDisplay
from drone_takevideo import TakeVideo
...
takevideo_max_memory = int(100e6)    # max 100MB
...
def ReceiveJoystickMessage(data):
    ...
    takevideo.handle(data.buttons[ButtonTakeVideo])
...
if __name__=='__main__':
    ...
    ButtonTakeVideo = int (   rospy.get_param("~ButtonTakeVideo",ButtonTakeVideo) )
    ...
    image_folder  = str ( rospy.get_param("~image_folder",image_folder) )
    takevideo_max_memory = int (eval( rospy.get_param("~takevideo_max_memory",takevideo_max_memory) ))

    # Now we construct our Qt Application and associated controllers and windows
    app = QtGui.QApplication(sys.argv)
    display = DroneVideoDisplay()
    takevideo = TakeVideo(display, image_folder, takevideo_max_memory)
