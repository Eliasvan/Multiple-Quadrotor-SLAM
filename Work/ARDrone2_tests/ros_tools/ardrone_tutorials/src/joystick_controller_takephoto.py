...
# and the DroneVideoDisplay class, which handles video display,
# and the TakePhoto class, which handles taking pictures of front and bottom cams.
from drone_controller import BasicDroneController
from drone_video_display2 import DroneVideoDisplay
from drone_takephoto import TakePhoto
...
# define the default image output directories
image_folder_front = "./output_front/"
image_folder_bottom = "./output_bottom/"
takephoto_live = True
takephoto_usb = False
...
def ReceiveJoystickMessage(data):
    ...
    takephoto.handle(data.buttons[ButtonTakePhoto])
...
if __name__=='__main__':
    ...
    rospy.init_node('ardrone_joystick_controller2')
    ...
    ButtonTakePhoto = int (   rospy.get_param("~ButtonTakePhoto",ButtonTakePhoto) )
    ...
    image_folder_front  = str ( rospy.get_param("~image_folder_front",image_folder_front) )
    image_folder_bottom = str ( rospy.get_param("~image_folder_bottom",image_folder_bottom) )
    takephoto_live = bool ( rospy.get_param("~takephoto_live",takephoto_live) )
    takephoto_usb = bool ( rospy.get_param("~takephoto_usb",takephoto_usb) )

    # Now we construct our Qt Application and associated controllers and windows
    app = QtGui.QApplication(sys.argv)
    display = DroneVideoDisplay()
    takephoto = TakePhoto(display, image_folder_front, image_folder_bottom, takephoto_live, takephoto_usb)
